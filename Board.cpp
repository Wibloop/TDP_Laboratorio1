...
//              Reserva memoria para el grid y el arreglo de bloques.
//              El grid se inicializa todo en 0 (celdas vacias).
// Parametros:
...
// Metodo: Constructor de copia -> Board
// Descripcion: Crea una copia profunda (deep copy) del Board.
//              Fundamental para A* donde cada nodo del arbol de busqueda
//              necesita su propio estado independiente del tablero.
//              Copia el grid celda por celda y cada bloque usando su copy constructor.
// Parametros:
//      - const Board& other: Referencia al Board a copiar
// Retorna: void
Board::Board(const Board& other) {
    ...
    // Deep copy del arreglo de bloques (cada Block tiene su propio copy constructor)
...
// Descripcion: Asigna el estado de otro Board a este, liberando memoria previa.
// Parametros:
//      - const Board& other: Board fuente
// Retorna:
//      - Board&: Referencia a este Board
...
// Metodo: Destructor -> ~Board
// Descripcion: Libera toda la memoria dinamica (grid y bloques)
// Parametros: Ninguno
// Retorna: void
Board::~Board() {
    delete[] grid;
...
// Metodo: getWidth
// Descripcion: Retorna el ancho del tablero
// Parametros: Ninguno
// Retorna: int width
...
// Metodo: getHeight
// Descripcion: Retorna el alto del tablero
// Parametros: Ninguno
// Retorna: int height
...
// Metodo: getStepLimit
// Descripcion: Retorna el limite de pasos
// Parametros: Ninguno
// Retorna: int stepLimit
int Board::getStepLimit() const {
    return stepLimit;
}
...
int Board::getCurrentStep() const {
    return currentStep;
}
...
Block& Board::getBlock(int index) {
    return blocks[index];
}
...
const Block& Board::getBlock(int index) const {
    return blocks[index];
}
...
// Retorna: void
void Board::setCurrentStep(int step) {
    currentStep = step;
}
...
// Metodo: isValidMove
// Descripcion: Verifica si el bloque en el indice dado puede moverse (dx, dy) celdas.
//              Para cada celda que el bloque ocuparia en la nueva posicion,
//              chequea que:
//              1. Este dentro de los limites del tablero
//              2. La celda destino este vacia (0) o ya pertenezca al mismo bloque
//              Esta funcion NO modifica ningun estado, solo consulta.
// Parametros:
...
bool Board::isValidMove(int blockIndex, int dx, int dy) const {
    ...
    int bh = b.getHeight();
    bool* geom = b.getGeometry();
    int blockGridVal = b.getID() + 10; // El valor que este bloque tiene en el grid
    ...
    for (int row = 0; row < bh; row++) {
        for (int col = 0; col < bw; col++) {
            ...
            if (geom != nullptr && geom[row * bw + col]) {
                ...
                int origX = bx + col;
                int origY = by + row;
                bool origOOB = (origX < 0 || origX >= width || origY < 0 || origY >= height);
...
//              El bloque permanece en el arreglo pero se marca con posicion (-1, -1)
//              para indicar que fue evacuado/removido.
//              Se usa cuando un bloque sale por una Exit.
// Parametros:
...
//              Preserva las paredes (valor 1) y re-estampa todos los bloques activos
//              (aquellos con posicion >= 0). Util despues de clonar un estado
//              y modificar posiciones de bloques directamente.
// Parametros: Ninguno
// Retorna: void
