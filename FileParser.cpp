...
void FileParser::parseMeta(const char* line) {
    if (startsWith(line, "NAME")) {
        ...
        int i = 0;
        while (line[i] != '=' && line[i] != '\0') i++;
        if (line[i] == '=') { i++; width = parseInt(line, i); }
    } else if (startsWith(line, "HEIGHT")) {
        int i = 0;
        while (line[i] != '=' && line[i] != '\0') i++;
        if (line[i] == '=') { i++; height = parseInt(line, i); }
    } else if (startsWith(line, "STEP_LIMIT")) {
        int i = 0;
        while (line[i] != '=' && line[i] != '\0') i++;
        if (line[i] == '=') { i++; stepLimit = parseInt(line, i); }
    }
}
...
void FileParser::parseBlock(const char* line) {
    ...
    while (line[pos] != '\0' && line[pos] != '\n' && line[pos] != '\r') {
        ...
        if (startsWith(line + pos, "COLOR=")) {
            ...
            pos += 7;
            initX = parseInt(line, pos); // Y en el archivo son las columnas (X interno)
...
// Descripcion: Procesa una fila del mapa de paredes. Cada '#' se agrega como pared.
void FileParser::parseWallLine(const char* line, int row) {
    for (int col = 0; line[col] != '\0' && line[col] != '\n' && line[col] != '\r' && col < width; col++) {
        if (line[col] == '#') {
...
void FileParser::parseExit(const char* line) {
    ...
    if (numExits >= exitCapacity) {
        ...
        for (int i = 0; i < numExits; i++) newExits[i] = exits[i];
        delete[] exits;
        exits = newExits;
    }
...
void FileParser::parseGate(const char* line) {
    ...
    if (numGates >= gateCapacity) {
        ...
        for (int i = 0; i < numGates; i++) newGates[i] = gates[i];
        delete[] gates;
        gates = newGates;
    }
...
// Metodo: parse
// Descripcion: Lee el archivo de nivel completo y construye el GameState inicial.
//   Recorre linea por linea, detecta secciones por marcadores [META], [BLOCK], etc.
//   y delega a los metodos especializados de parseo.
//   Retorna un GameState* con el tablero, bloques y salidas configurados.
// Parametros:
...
GameState* FileParser::parse(const char* filename) {
    ...
    while (file.getline(line, 512)) {
        ...
        if (startsWith(line, "[META]")) { section = 1; continue; }
        if (startsWith(line, "[BLOCK]")) { section = 2; break; }
        if (section == 1 && line[0] != '\0') parseMeta(line);
    }
    ...
    while (file.getline(line, 512)) {
        ...
        if (startsWith(line, "[BLOCK]")) { section = 2; continue; }
        if (startsWith(line, "[WALL]")) { section = 3; wallRow = 0; continue; }
        if (startsWith(line, "[EXIT]")) { section = 4; continue; }
        if (startsWith(line, "[GATE]")) { section = 5; continue; }

        if (line[0] == '\0') continue; // Linea vacia

        switch (section) {
            case 2: parseBlock(line); break;
            case 3: parseWallLine(line, wallRow++); break;
            case 4: parseExit(line); break;
            case 5: parseGate(line); break;
        }
    }
    ...
    // Nota: Las paredes de borde NO se abren para las salidas.
    // La evacuacion se verifica logicamente (bloque adyacente al borde con color correcto).
